...
def get_weather(city):
    api_key = "a26c6308c89801c4e2132194681e8bc9"
    url = f"http://api.openweathermap.org/data/2.5/weather?q={city}&appid={api_key}"

    try:
        data = requests.get(url).json()
        temp = data["main"]["temp"] - 273.15
        return f"{city} temperature is {round(temp,2)} degree Celsius"
    except:
...
def run_assistant():
    ...
    if any(word in command for word in ["hello", "hi", "hey"]):
        speak("Hello Gaja, how can I help you?")
...
#  UI 
root = tk.Tk()
root.title("Voice Assistant - Gaja")
